  ...
  void computeDensity()
  {
    // TODO:
      ...
      for(tIndex i=0; i<particleCount(); ++i) {
        ...
        //gi is i-coordinate of the neighboring cell, gj is the j-coordinate of the neighboring cell
        ...
        for(int gj=std::max(0, gj_from); gj<std::min(resY(), gj_to); ++gj) {
            for(int gi=std::max(0, gi_from); gi<std::min(resX(), gi_to); ++gi) {
                //you are iterating over the adjacent cells
                const tIndex gidx = idx1d(gi, gj);
                //is just the index of the cell in the array representing the grid cells
      
                //and then, you can iterate over all the particles that are stored in this cell, using:
